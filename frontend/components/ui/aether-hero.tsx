...
export type AetherHeroProps = {
    ...
    fixedBackground?: boolean; // if true, canvas is fixed behind content
};
...
float scene(vec2 uv) {
  ...
  for (float i=.0; i<3.; i++) {
    intensity+=pattern(uv+i*6./MN);
  }
  return intensity;
}
void main() {
  // Move origin to the bottom left slightly off-screen
  vec2 origin = vec2(-0.1 * R.x, -0.1 * R.y);
  vec2 uv = (FC - origin)/MN;
  
  // Center glowing effect
  float s=12., e=4e-4;
  float intensity=e/(sin(uv.x*s)*cos(uv.y*s));
  
  intensity+=scene(uv);
  
  // Soft fade out towards the top/right to look less annoying
  float dist = length(FC/MN);
  intensity *= 1.0 - smoothstep(0.0, 3.0, dist);
  intensity *= 0.6; // Reduce overall brightness
  
  // ScholarHub Cool Blue theme
  // RGB: 25, 128, 255 -> 0.1, 0.5, 1.0 (Deep vibrant blue to cyan)
  vec3 colorTheme = vec3(0.1, 0.5, 1.0); 
  vec3 col = colorTheme * intensity * 2.5; // boost slightly since we removed multi-channel addition
...
export default function AetherHero({
    ...
    return (
        <section
            ...
            style={{
                height: fixedBackground ? 'auto' : height,
                minHeight: fixedBackground ? '100vh' : undefined,
                position: 'relative',
                overflow: fixedBackground ? 'visible' : 'hidden'
            }}
            aria-label="Hero"
        >
            {/* Font import (Space Grotesk) - Optional but provided by default */}
            <style jsx global>{`
      ...
      `}</style>
            ...
            <div style={{
                position: fixedBackground ? 'fixed' : 'absolute',
                inset: 0,
                width: fixedBackground ? '100vw' : '100%',
                height: fixedBackground ? '100vh' : '100%',
                zIndex: 0,
                pointerEvents: 'none'
            }}>
